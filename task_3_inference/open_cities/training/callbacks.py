...
class ModelCheckpoint(Callback):
    def __init__(
        ...
        self.top_k_template = (
            os.path.join(directory, "k-ep[{epoch}]-{%s:.4f}.pth") % monitor
        )

        self._top_k: List[Tuple[float, str]] = []
        self._compare_fn = (
            operator.gt if mode == "max" else operator.lt
        )  # first current!
    ...
    def reset(self):
        self._top_k: List[Tuple[float, str]] = []
        self._compare_fn = (
            operator.gt if self.mode == "max" else operator.lt
        )  # first current!
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if self.save_top_k:
            ...
            if len(self._top_k) < self.save_top_k:
                ...
                self._top_k = list(
                    sorted(self._top_k, key=lambda x: x[0], reverse=self._reverse)
                )
